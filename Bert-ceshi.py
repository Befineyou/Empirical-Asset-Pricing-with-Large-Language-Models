...
#     return texts, labels
def df2datasets(df, label=True, integer=False, json_filename=None):
    texts = []
    labels = []

    # 如果提供了json_filename，则从JSON文件中解析Prompts
    if json_filename:
    ...
    for index, row in df.iterrows():
        if json_filename:
            ...
            for feature in row.index[:-1]:  # 假设最后一列是目标变量
                value = row[feature]
                prompt = prompt.replace(f"{feature}=", f"{feature}={value:.4f}")
        else:
            # 生成新的Prompt
            prompt, completion = data2text(row, label, integer)
...
def run_test(model_path, test_data_path):
    config = {'model_name': 'bert-base-uncased', "num_epochs": 5, "batch_size": 10, "max_length": 512}
    ...
    json_filename = "output.jsonl"
    test_texts, test_labels = df2datasets(test_df, label=True, json_filename=json_filename)

    #test_texts, test_labels = df2datasets(test_df, label=False)

    # 评估模型
    bert_fine_tuner = BertFineTuner(config=config, train_texts=[], train_labels=[], valid_texts=[], valid_labels=[])
    bert_fine_tuner.model = model
    bert_fine_tuner.tokenizer = tokenizer

    y_test_outputs, _, _, rmse, rmse_woo = bert_fine_tuner.eval(test_texts=test_texts, test_labels=test_labels)
